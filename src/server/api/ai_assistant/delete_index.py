...
def deleteIndex(collection,index,logger):
    try:
        logger.info("initiate delete index")
        if index in collection.getIndexes():
            collection.drop_search_index(index)

        logger.info(f"deleted index:{index}")

    except Exception as e:
        logger.error(f"error in deleting the index: {e}")
        raise
